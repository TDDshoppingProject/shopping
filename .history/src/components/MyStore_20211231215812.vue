...
export default {
  ...
  methods: {
    ...
    // 修改商品信息
    updateGoodsInfo () {
      this.$axios.post('/business/setgoods/' + window.sessionStorage.getItem('userid') + '/' + this.goodsEditing.name + '/' + this.goodsEditing.price + '/' + this.goodsEditing.img)
        .then(res => {
          console.log(res)
        })
    },
