...
export default {
  ...
  methods: {
    ...
    // 上传商品
    uploadGoods () {
      console.log(this.goodsEditing.img)
      this.$axios.post('/business/goodsupload/' + window.sessionStorage.getItem('userid') + '/' + this.goodsEditing.price, this.goodsEditing.name, this.goodsEditing.img)
        .then(res => {
          console.log(res)
          if (res.data.status === 200) {
    ...
    // 修改商品信息
    updateGoodsInfo () {
      this.$axios.post('/business/setgoods/' + this.gid + '/' + this.goodsEditing.price, this.goodsEditing.name, this.goodsEditing.img)
        .then(res => {
          console.log(res)
        })
    },
