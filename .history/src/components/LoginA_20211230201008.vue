        ...
        <div class="loginBox">
            ...
            <el-form ref="loginFormRef" class="loginForm" :rules="loginFormRules" :model="loginForm">
                <el-form-item prop="account">
                    < v-model="loginForm.account" prefix-icon="iconfont icon-avatar" placeholder="请输入用户名"></>
               </el-form-item>
        ...
        if (valid) {
          ...
          if (res.status !== 200) {
           ...
           1.2.token只应在当前网站打开期间生效, 所以将token保存到sessionStorage中 */
            ...
            // 将用户角色存储在role中，1为商家，0为普通用户
            window.sessionStorage.setItem('role', res.data.user.business)
          }
        } else {
          return false
        }
