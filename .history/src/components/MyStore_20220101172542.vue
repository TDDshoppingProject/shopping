            ...
            <div class="bottom" >
              <el-tooltip content="修改商品信息" placement="bottom">
                <el-button
                  type="primary"
                  icon="el-icon-edit"
                  @click="alterGoodsDialog(item.id,item.name,item.price)"
                  circle
                ></el-button>
              </el-tooltip>
    ...
    >
      ...
      >
        ...
        <el-form-item label="商品名称" prop="name">
          <el-input v-model="goodsupload.name"></el-input>
        </el-form-item>
        <!-- 上传价格 -->
        <el-form-item label="价格" prop="price">
          <el-input v-model.number="goodsupload.price"></el-input>
        </el-form-item>
        <!-- 上传图片 -->
        <el-form-item label="图片"  prop="img">
          <el-input
            v-model="goodsupload.img"
            autocomplete="off"
            placeholder="图片 URL"
          ></el-input>
...
export default {
  ...
  methods: {
    ...
    addImage: function () {
      ...
      if (self.$refs.new_image.files.length !== 0) {
        ...
        self.$axios.post('/addImage', file, {
          ...
          console.log(res)
          // 上传时
          if (this.goodsDialogVisible === true) {
            this.goodsupload.img = res.data
          }
          // 修改时
          if (this.alterGoods === true) {
    ...
    // 上传商品
    uploadGoods () {
      console.log(this.goodsupload.img)
      this.$axios.post('/business/goodsupload/' + window.sessionStorage.getItem('userid') + '/' + this.goodsupload.name + '/' + this.goodsupload.price, this.goodsupload.img)
        .then(res => {
          console.log(res)
          if (res.data.status === 200) {
    ...
    alterGoodsDialog (gid, name, price) {
      this.gid = gid
      this.alterGoods = true
      this.goodsEditing.name = name
      this.goodsEditing.price = price
