<template v-if="window.sessionStorage.getItem('role') === '1'">
  <div>
    <!-- 调节大小 -->
    ...
    <!-- 商品信息 -->
    <el-row>
      <el-col :span="4" v-for="(item, index) in goodsForm" :key="item" :offset="index > 0 ? 2 : 0">
        <el-card class="card">
  ...
  methods: {
    updateUserInfo () {
      ...
      this.$refs.storeFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.put('/users', this.storeForm);
